...
class ResultRanker:
    ...
    def rank(
        ...
        INELIGIBLE schemes are always included (with low confidence) so
        build_engine_response can count and handle them correctly.
        """
        ranked = []
        ...
        for scheme, elig_out in scheme_results:
            ...
            if elig_out.result == INELIGIBLE:
                # Always include INELIGIBLE so meta counts are accurate
                # build_engine_response will NOT surface these to the user
                dl, al = "Not Eligible", "See Why"
            ...
            # For non-ineligible: apply UX visibility filter
            if c < THRESH_MAYBE and not elig_out.missing_fields:
        ...
        # Sort: eligible first, then possible by confidence desc, ineligible last
