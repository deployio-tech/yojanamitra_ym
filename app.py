...
from app.engine.scorer import ResultRanker, RankedScheme, THRESH_ELIGIBLE, THRESH_POSSIBLE, THRESH_MAYBE
    ...
    try:
        ...
        if sms_gateway_url:
            ...
            if "smsmobileapi.com" in sms_gateway_url:
                ...
                final_url = sms_gateway_url if "sendsms" in sms_gateway_url else "https://api.smsmobileapi.com/sendsms/"
                data = {
                    "apikey": sms_gateway_key,
                    "recipients": phone_number.replace('+', ''), # Usually expects no + for this API
                    "message": message
                }
            else:
            ...
            try:
                ...
                res = requests.post(final_url if "smsmobileapi.com" in sms_gateway_url else sms_gateway_url, json=data, timeout=10)
                
                if res.status_code in [200, 201]:
                    print(f"✅ Custom Gateway SMS sent to {phone_number}")
                    return True
                else:
                    print(f"📡 SMS Debug: JSON POST failed ({res.status_code}): {res.text[:200]}")
                    # Try form-data if JSON fails
                    res_form = requests.post(final_url if "smsmobileapi.com" in sms_gateway_url else sms_gateway_url, data=data, timeout=10)
                    if res_form.status_code in [200, 201]:
                        print(f"✅ Custom Gateway SMS sent to {phone_number} (form-data)")
                        return True
                    
                    print(f"📡 SMS Debug: Form POST failed ({res_form.status_code}): {res_form.text[:200]}")

                    # Try GET as last resort for some gateways
                    res_get = requests.get(final_url if "smsmobileapi.com" in sms_gateway_url else sms_gateway_url, params=data, timeout=10)
                    if res_get.status_code in [200, 201]:
                        print(f"✅ Custom Gateway SMS sent to {phone_number} (GET request)")
                        return True
                    
                    print(f"📡 SMS Debug: GET failed ({res_get.status_code}): {res_get.text[:200]}")
                    ...
                    if res_alt.status_code in [200, 201]:
                        print(f"✅ Custom Gateway SMS sent to {phone_number} (alt format)")
                        return True
                    print(f"⚠️ Gateway Relay failed (Status {res.status_code or res_form.status_code}), falling back to Twilio...")
...
def get_config():
    ...
    return jsonify({
        'google_client_id': os.getenv('GOOGLE_CLIENT_ID', "886008127383-v80l90m0uukpgov20n0223n748ivcc5d.apps.googleusercontent.com")
    })
...
def get_profile_for_desktop():
    ...
    field_map = {
        ...
        'Annual Family Income': ('Rs.' + str(int(user.annual_family_income))) if user.annual_family_income else None,
        'Caste': user.caste,
...
def google_auth():
    ...
    try:
        ...
        CLIENT_ID = os.getenv('GOOGLE_CLIENT_ID', "886008127383-v80l90m0uukpgov20n0223n748ivcc5d.apps.googleusercontent.com")
...
def view_document(doc_id):
    ...
    user_id = session.get('user_id')
    print(f"📡 view_document: doc_id={doc_id}, session user_id={user_id}, user_type={session.get('user_type')}")
...
def api_classify_schemes():
    ...
    try:
        ...
        rows = conn.execute("SELECT scheme_id, extracted_json FROM gemini_prefill WHERE status='success'").fetchall()
        conn.close()
    except Exception as e:
...
class User(db.Model):
    ...
    achievement_certificates = db.Column(db.Text)  # JSON list e.g. ["sports_certificate","ncc_certificate"]
    ...
    question_answers = db.Column(db.Text, default='{}')  # JSON dict of field -> answer mappings
...
class Scheme(db.Model):
    ...
    FLAT_COLUMNS = {
        ...
        'allowed_states', 'allowed_education', 'allowed_occupations',
        'allowed_marital_status', 'min_income', 'max_income',
        'disability_requirement', 'residence_requirement',
        'minority_requirement', 'senior_citizen_requirement',
        'widow_requirement', 'orphan_requirement', 'tribal_requirement',
        'land_type_requirement', 'bank_account_required', 'aadhaar_required',
        'allowed_father_occupations', 'allowed_mother_occupations',
        'allowed_religions', 'allowed_ration_card_types',
        'disability_percentage_min', 'min_education_level'
    }
...
class Condition(db.Model):
    ...
    source          = db.Column(db.String(32), default="manual")  # 'manual', 'extraction', 'migration'
    is_ambiguous    = db.Column(db.Boolean, default=False)
    notes           = db.Column(db.Text)
    created_at      = db.Column(db.DateTime, default=datetime.utcnow)

    scheme = db.relationship('Scheme', backref=db.backref('condition_rows', lazy='dynamic', cascade='all, delete-orphan'))
...
class EligibilityResult(db.Model):
    ...
    scheme = db.relationship('Scheme', backref=db.backref('elig_results', lazy='dynamic'))
...
class SchemeClarification(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'scheme_id', 'question_id_hash', name='uq_clarification'),
    )
...
class Feedback(db.Model):
    ...
    scheme = db.relationship('Scheme', backref=db.backref('feedback_list', lazy='dynamic'))
...
def _user_get_profile_dict(self):
    base = {
        ...
        'is_student':       _str_to_bool(normalize_is_student(
                               self.education_status, self.employment_status,
...
def submit_question_answer():
    ...
    if expected_type in ('integer', 'number'):
        try:
            ...
            app.logger.warning(f"[VALIDATION] Invalid numeric value '{value}' for field '{field}' (expected {expected_type})")
    ...
    try:
        ...
        if existing_attr:
            ...
            db.session.add(UserProfileAttribute(
                user_id=user_id, field=field, value=encoded_val,
                source='question_answer', confidence=1.0
            ))
        ...
        if canonical != field:
            ...
            if existing_canon:
                ...
                db.session.add(UserProfileAttribute(
                    user_id=user_id, field=canonical, value=encoded_val,
                    source='question_answer', confidence=1.0
                ))
    ...
    try:
        ...
        for field, value in answers.items():
            ...
            if existing_attr:
                ...
                db.session.add(UserProfileAttribute(
                    user_id=user_id, field=field, value=encoded_val,
                    source='bulk_resolve', confidence=1.0
                ))
        ...
        for sid in scheme_ids:
            ...
            if result.result == 'eligible':
                ...
                newly_eligible_names.append(scheme.name)
                 ...
                 impact_reason = result.blocking_reason or "One or more mandatory requirements failed."
        ...
        return jsonify({
            ...
            'total_resolved': len(newly_eligible_names) + len(newly_ineligible_names)
        }), 200
...
def generate_resolve_questions():
    ...
    try:
        ...
        SKIP_COLS = {'id', 'email', 'password_hash', 'created_at', 'updated_at', 'is_admin', 
                     'profile_version', 'question_answers'}
        
        from app.engine.eligibility import get_profile_value
        user_profile_dict = user.get_profile_dict() if hasattr(user, 'get_profile_dict') else {}
        ...
        for col in User.__table__.columns.keys():
            ...
            if resolved_val is not None and resolved_val != '' and resolved_val != '{}' and resolved_val != '[]':
                # Convert to human-readable for AI
                if isinstance(resolved_val, (int, bool)) and (col.startswith(('is_', 'has_', 'own_')) or 'available' in col):
        ...
        for s in schemes_info:  # Removed cap to analyze all schemes
            scheme_context.append({
                'id': s.get('id'),
                'name': s.get('name', ''),
                'reason': s.get('questions', '') or s.get('reason', '') or 'Unknown verification needed',
...
def sync_profile_from_vault():
    ...
    for doc in documents:
        ...
        if 'ration card' in dtype or 'bpl card' in dtype:
            ...
            rc_type = ex.get('Ration Card Type') or ex.get('Card Type') or ex.get('Category')
            ...
            if str(doc_key).startswith('YM-Q-'):
                ...
                try:
                    ...
                    if text_val in ['yes', 'y', 'shaded', 'ticked']:
                        ...
                        clean_val = 'No'
                    elif field_name in ['age', 'income', 'annual_family_income', 'annual_income']:
                        try:
        ...
        if 'domicile' in dtype or 'residence certificate' in dtype or 'nativity' in dtype:
        ...
        if 'disability' in dtype or 'divyang' in dtype or 'pwd' in dtype:
            ...
            dis_pct = ex.get('Disability Percentage') or ex.get('Percentage') or ex.get('disability_percentage')
            if dis_pct and not user.disability_percentage:
                try:
                    user.disability_percentage = int(str(dis_pct).replace('%', '').strip())
    ...
    if achievement_certs:
        existing = []
        try:
            existing = json.loads(user.achievement_certificates) if user.achievement_certificates else []
        except Exception:
            pass
...
def _normalize_doc_type(raw_type: str) -> str:
    ...
    if not raw_type:
        return ""
    return VAULT_DOC_TYPE_MAP.get(raw_type.lower().strip(), raw_type.lower().strip().replace(" ", "_"))

def _build_audit_trail(scheme, user, ai_score, ai_verdict, ai_reason,
                       ai_errors, engine_result=None) -> dict:
    """
    ...
    engine_layer = {
        'passed_conditions': [], 'failed_conditions': [],
        'soft_failed_conditions': [], 'missing_mandatory_docs': [],
        'missing_optional_docs': [], 'confidence_score': None,
        'eligibility_class': None,
    }
    if engine_result:
        engine_layer['confidence_score'] = getattr(engine_result, 'confidence_score', None)
        engine_layer['eligibility_class'] = str(getattr(engine_result, 'eligibility_class', ''))
        for c in getattr(engine_result, 'passed_conditions', []):
            engine_layer['passed_conditions'].append({
                'field': c.condition_field, 'user_value': str(c.user_value),
                'expected': str(c.expected_value), 'operator': c.operator,
            })
        for c in getattr(engine_result, 'failed_conditions', []):
            engine_layer['failed_conditions'].append({
                'field': c.condition_field, 'user_value': str(c.user_value),
                'expected': str(c.expected_value), 'operator': c.operator,
                'message': c.failure_message, 'mandatory': c.is_mandatory,
            })
        for c in getattr(engine_result, 'soft_failed_conditions', []):
            engine_layer['soft_failed_conditions'].append({
                'field': c.condition_field, 'message': c.failure_message,
            })
        engine_layer['missing_mandatory_docs'] = getattr(engine_result, 'missing_mandatory_docs', [])
        engine_layer['missing_optional_docs']  = getattr(engine_result, 'missing_documents', [])

    scheme_docs_required = _docs_required_from_text(getattr(scheme, 'documents_required', '') or '')
    ...
    vault_docs   = [d.doc_type for d in user.documents if d.doc_type] if hasattr(user, 'documents') and user.documents else []
    ...
    profile_snapshot = {
        ...
        'education': getattr(user, 'highest_education_level', None) or getattr(user, 'education', None),
    ...
    verdict_text = {'ELIGIBLE': 'fully eligible', 'POSSIBLY_ELIGIBLE': 'possibly eligible',
                    'NOT_ELIGIBLE': 'not eligible'}.get(ai_verdict, ai_verdict)
    summary_parts = [
        f"User is {verdict_text} for \'{scheme.name}\' (AI score: {ai_score}/100).",
        f"Engine: {passed_count} condition(s) passed" + (f", {failed_count} failed." if failed_count else "."),
    ]
    if ai_reason:
        summary_parts.append(f"AI: {ai_reason}")
    if docs_missing:
        summary_parts.append(f"Missing docs: {', '.join(d.replace('_',' ').title() for d in docs_missing)}.")
    if ai_errors:
        summary_parts.append(f"Hard failures: {'; '.join(ai_errors)}.")

    return {
        'evaluated_at': now, 'scheme_id': scheme.id, 'scheme_name': scheme.name,
        'ai_score': ai_score, 'ai_verdict': ai_verdict, 'ai_reason': ai_reason,
        'ai_errors': ai_errors, 'engine_layer': engine_layer,
        'documents': {
            'in_vault': vault_docs, 'required_by_scheme': scheme_docs_required,
            'present': docs_present, 'missing': docs_missing,
        },
...
def _parse_achievement_list(certs: list, cert_type: str) -> list:
    ...
    prefix_map = {
        'sports': ['sports', 'game', 'athlete', 'olympic', 'commonwealth', 'national_sports', 'state_sports', 'district_sports'],
        'art': ['art', 'cultural', 'dance', 'music', 'painting', 'fine_arts', 'classical', 'performing_arts'],
        'skill': ['skill', 'iti', 'vocational', 'apprentice', 'trade', 'competency', 'professional_cert'],
    }
...
def _build_documents_uploaded(user) -> list:
    ...
    if getattr(user, 'caste', None) and getattr(user, 'caste', '').lower() not in ['general', 'open', '']:
...
def save_profile():
    ...
    try:
        ...
        for col, col_obj in User.__table__.columns.items():
            if col in ['id', 'email', 'password_hash', 'created_at', 'updated_at', 'is_admin']:
                continue
            ...
            if val is not None:
                ...
                if isinstance(val, str):
                    ...
                    if v_low in ['yes', 'true', 'y', '1']:
                        normalized_val = "Yes" if 'INTEGER' not in col_type and 'FLOAT' not in col_type else 1
                    elif v_low in ['no', 'false', 'n', '0']:
                        normalized_val = "No" if 'INTEGER' not in col_type and 'FLOAT' not in col_type else 0
                    elif v_low == '':
                        normalized_val = None

                if 'INTEGER' in col_type:
                    try:
                        setattr(user, col, int(normalized_val) if normalized_val is not None else None)
                    except: 
                        logger.warning(f"Failed to cast {col} value '{val}' to INTEGER")
                elif 'FLOAT' in col_type or 'REAL' in col_type:
                    try:
                        setattr(user, col, float(normalized_val) if normalized_val is not None else None)
                    except:
                        logger.warning(f"Failed to cast {col} value '{val}' to FLOAT")
                elif ('TEXT' in col_type or 'VARCHAR' in col_type) and isinstance(val, (list, dict)):
...
def get_schemes():
    ...
    return jsonify({
        'schemes': [{
            ...
            'matchPercentage': 50,
            'conflicts':       json.loads(s.mutually_exclusive_with) if s.mutually_exclusive_with else [],
        } for s in paginated],
        'page':        page,
        'limit':       limit,
...
def translate_text_api():
    ...
    lang_map = {'hi': 'Hindi', 'kn': 'Kannada', 'ml': 'Malayalam', 'ta': 'Tamil', 'te': 'Telugu'}
...
def distill_questions_api():
    ...
    for idx, q in enumerate(raw_questions):
        ...
        formatted_list += f"[{idx}] (Context: {q.get('field', 'General')}) {q['question']}\n"

    # 2. Build the reinforced Consolidation Prompt
    # We remove privacy sensitive fields from log
    profile_summary = ", ".join([f"{k}: {v}" for k, v in current_profile.items() if v is not None and k not in ('id', 'password', 'key')])
    ...
    try:
        ...
        fallback = [{"uid": f"YM-Q-{i+1}", "text": q['question'], "answerType": "yesno", "matches": [i]} for i, q in enumerate(raw_questions)]
...
def analyze_scheme_readiness_ai(scheme_id):
    ...
    user_data = user.get_profile_dict() if hasattr(user, 'get_profile_dict') else user.to_dict().get('profile', {})
    ...
    clarifications = SchemeClarification.query.filter_by(user_id=user_id, scheme_id=scheme_id).all()
    clarification_text = ""
    if clarifications:
    ...
    try:
        ...
        if 'items' in result:
            for item in result['items']:
                if item.get('type') in ['warning', 'partial'] and item.get('question'):
                    h_str = f"{scheme_id}_{item.get('title','')}_{item.get('question','')}".encode('utf-8')
        ...
        try:
            ...
            for cr in (eo.condition_results or []):
                ...
                if cr.status == PASS_R:
                    items.append({'title': title, 'text': cr.reason or 'Passed', 'type': 'success', 'icon': 'fa-circle-check'})
                elif cr.status == FAIL_R:
                    items.append({'title': title, 'text': cr.reason or 'Failed', 'type': 'error', 'icon': 'fa-times-circle'})
            for doc in eo.acquirable:
                items.append({'title': 'Document Required', 'text': f'Missing: {doc}', 'type': 'warning', 'icon': 'fa-file-circle-exclamation'})
            score = 100
            ...
            return jsonify({
                ...
                'readiness_summary': eo.blocking_reason or (eo.top_insight if hasattr(eo, 'top_insight') else ''),
            }), 200
...
def _auto_convert_to_conditions(scheme, data, source='manual'):
    ...
    disability_req = data.get('disabilityRequirement') or data.get('disability_requirement')
    ...
    residence_req = data.get('residenceRequirement') or data.get('residence_requirement')
    ...
    fo = _j(data.get('allowedFatherOccupations') or data.get('allowed_father_occupations'))
    if fo:
        _add_cond('father_occupation', 'in', fo, 'soft', 0.80)
    mo = _j(data.get('allowedMotherOccupations') or data.get('allowed_mother_occupations'))
    if mo:
    ...
    senior_req = data.get('seniorCitizenRequirement') or data.get('senior_citizen_requirement')
    ...
    disability_pct = data.get('disabilityPercentageMin') or data.get('disability_percentage_min')
    ...
    # Ration card types
    rc_types = _j(data.get('allowedRationCardTypes') or data.get('allowed_ration_card_types'))
    if rc_types:
...
def normalize_education_level(raw_val, fallback_status=None):
    ...
    if any(x in combined for x in ["phd","ph.d","doctorate","doctoral"]):
        return "phd"
    if any(x in combined for x in ["post grad","postgrad","pg ","m.tech","mtech","mba","mca",
                                     "master","m.e ","m.sc","msc","ma ","m.a","post-grad","m.e."]):
        return "postgrad"
    if any(x in combined for x in ["be","b.e","btech","b.tech","bsc","b.sc","bca","bba","ba ",
                                     "b.a","llb","mbbs","bachelor","graduation","graduate","degree",
                                     "ug ","under grad","undergrad","b.com","bcom","currently pursuing",
                                     "pursuing","engineering","college"]):
        return "graduation"
    if any(x in combined for x in ["diploma","polytechnic","iti"]):
        return "diploma"
    if any(x in combined for x in ["12th","hsc","puc","higher secondary","intermediate","10+2","class 12"]):
        return "secondary"
    if any(x in combined for x in ["10th","sslc","matriculation","class 10","primary","8th"]):
        return "primary"
    return "none"

def normalize_is_student(education_status, employment_status, education_level_raw,
                         occupation=None):
    """
    ...
    STUDENT_SIGNALS = [
        "student", "studying", "currently studying", "pursuing",
        "currently pursuing", "enrolled", "schooling", "college going",
        "in school", "in college", "in university",
    ]
    NON_STUDENT_OVERRIDES = [
        "employed", "working", "job", "service", "business",
...
def get_recommendations():
    ...
    if not user or not user.age:
        return jsonify({'recommendations': [], 'possibly_eligible': [], 'message': 'Complete your profile for recommendations', 'meta': {'total_evaluated': 0, 'fully_eligible': 0, 'possibly_eligible': 0, 'not_eligible': 0}}), 200
    ...
    raw_profile = {
        ...
        "is_govt_employee":      str(user.occupation or "").lower() in ["government employee", "govt employee", "government servant", "government service"],
        "is_income_taxpayer":    False,
        "is_student":            normalize_is_student(user.education_status, user.employment_status, user.highest_education_level or user.education, user.occupation),
        ...
        "education_level":       normalize_education_level(user.highest_education_level or user.education, user.education_status),
        ...
        "sports_certificates": _parse_achievement_list(_build_achievement_certs(user), 'sports'),
        "art_certificates": _parse_achievement_list(_build_achievement_certs(user), 'art'),
        "ncc_certificate": _check_achievement_flag(_build_achievement_certs(user), 'ncc'),
        "nss_certificate": _check_achievement_flag(_build_achievement_certs(user), 'nss'),
        "skill_certificates": _parse_achievement_list(_build_achievement_certs(user), 'skill'),
        # G-series gap fixes
        "scheme_previously_availed": user.scheme_previously_availed == "Yes" if hasattr(user, "scheme_previously_availed") and user.scheme_previously_availed else False,
    ...
    try:
        ...
        return jsonify({
            ...
            "recommendations": [],
            "possibly_eligible": [],
            "meta": {"total_evaluated": 0, "fully_eligible": 0, "possibly_eligible": 0, "not_eligible": 0}
        }), 500
...
def deep_eligibility_search():
    ...
    user = db.session.get(User, user_id)
    if not user or not user.age:
        return jsonify({'error': 'Please complete your profile first (Age is required)'}), 400
    ...
    BLOCKED = {'name', 'email', 'mobile', 'aadhaar', 'pan', 'dob', 'id',
               'aadhaarLinkedBank', 'mobileLinkedBank', 'bankAccountNumber'}
    user_data_clean = {k: v for k, v in user_data.items()
...
[{{"id": 1, "score": 85, "verdict": "ELIGIBLE", "reason": "...", "errors": []}}, ...]"""
    ...
    for i in range(0, len(candidates), BATCH_SIZE):
        ...
        try:
            ...
            for s in batch:
                eng_result = next(
                    (r for r in engine_response.fully_eligible if int(r.scheme_id) == s.id), None
                )
    ...
    logger.info(f"Deep search complete: {len(results)} schemes passed threshold {threshold}%")
...
def privatize_profile(raw: dict) -> dict:
    ...
    if age is not None:
        age = int(age)
        if   age <  6:  age_bracket = "0–5"
        elif age < 12:  age_bracket = "6–11"
        elif age < 18:  age_bracket = "12–17"
        elif age < 26:  age_bracket = "18–25"
        elif age < 36:  age_bracket = "26–35"
        elif age < 46:  age_bracket = "36–45"
        elif age < 56:  age_bracket = "46–55"
        elif age < 61:  age_bracket = "56–60"
        else:            age_bracket = "60+"
    ...
    if income is not None:
        income = int(income)
        if   income <=  100_000: income_bracket = "EWS (up to ₹1L/yr)"
        elif income <=  300_000: income_bracket = "LIG (₹1L–3L/yr)"
        elif income <=  800_000: income_bracket = "MIG (₹3L–8L/yr)"
        else:                     income_bracket = "HIG (above ₹8L/yr)"
    ...
    if dis_pct is not None:
        dis_pct = int(dis_pct)
        if   dis_pct == 0:   dis_bracket = "none"
        elif dis_pct < 40:   dis_bracket = "below 40%"
        elif dis_pct < 60:   dis_bracket = "40–59%"
        elif dis_pct < 80:   dis_bracket = "60–79%"
        else:                 dis_bracket = "80–100%"
    ...
    if land is not None:
        land = float(land)
        if   land == 0:    land_bracket = "none"
        elif land <  1:    land_bracket = "marginal (under 1 acre)"
        elif land <  5:    land_bracket = "small (1–5 acres)"
        elif land < 10:    land_bracket = "medium (5–10 acres)"
        else:               land_bracket = "large (10+ acres)"
    ...
    NON_EMPLOYED = {"", "none", "housewife", "homemaker", "unemployed",
                    "retired", "deceased", "n/a", "na", "not applicable"}
...
def run_deep_analysis():
    ...
    try:
        ...
        if not target_schemes:
            return jsonify({'verified_eligible': scheme_ids, 'moved_to_possibly': [], 'ineligible_count': 0}), 200
...
def refresh_recommendations_with_readiness():
    ...
    if not user or not user.age:
        return jsonify({
            ...
            'meta': {'total_evaluated': 0, 'candidates': 0, 'passed_readiness': 0, 'removed_low_readiness': 0}
        }), 200
    ...
    try:
        ...
        logger.info(f"Refresh complete: {passed_count} passed readiness >= {READINESS_THRESHOLD}%, {removed_count} removed")
...
def readiness_reclassify():
    ...
    scheme_ids = data.get('scheme_ids', [])
    if not scheme_ids:
        return jsonify({'keep_eligible': [], 'move_to_possibly': [], 'move_to_ineligible': []}), 200

    # Build clean user profile for AI (strip PII)
    user_data = user.to_dict().get('profile', {})
    BLOCKED_KEYS = {'name', 'email', 'mobile', 'aadhaar', 'pan', 'dob', 'id',
                    'aadhaarLinkedBank', 'mobileLinkedBank', 'bankAccountNumber'}
    user_data_clean = {k: v for k, v in user_data.items()
    ...
    BATCH_SIZE = 6
    schemes_map = {s.id: s for s in Scheme.query.filter(Scheme.id.in_(scheme_ids)).all()}
            ...
            for r in batch_results:
                ...
                for item in items:
                    if item.get('type') == 'warning' and item.get('question'):
                        h_str = f"{sid}_{item.get('title','')}_{item.get('question','')}".encode('utf-8')
                        item['question_id'] = hashlib.md5(h_str).hexdigest()
...
def readiness_reevaluate():
    ...
    if iteration_count >= MAX_ITERATIONS:
        return jsonify({"status": "throttled", "reason": "max_iterations_exceeded"}), 429

    # 2. Re-Evaluate Prompt Execution
    user_data = user.to_dict().get('profile', {})
    BLOCKED_KEYS = {'name', 'email', 'mobile', 'aadhaar', 'pan', 'dob', 'id', 'aadhaarLinkedBank'}
    profile_safe = {k: v for k, v in user_data.items() if v and k not in BLOCKED_KEYS and 'Number' not in k}
    prior_clarifications = SchemeClarification.query.filter_by(user_id=user_id, scheme_id=scheme_id).all()
    prior_text = "\n".join([f"Q: {c.question_text} \nA: {c.answer_text}" for c in prior_clarifications]) if prior_clarifications else "None"
...
{{"verdict": "ELIGIBLE", "justification": "..."}}"""
    ...
    for a in answers:
        ...
        existing = SchemeClarification.query.filter_by(user_id=user_id, scheme_id=scheme_id, question_id_hash=q_hash).first()
        if not existing:
...
def contextual_ai():
    ...
    PII_KEYS = {'aadhaar', 'pan', 'email', 'mobile', 'phone', 'dob',
                'user_id', 'id', 'name', 'bank_account', 'account_number',
...
def get_verified_cache():
    ...
    if not user:
        return jsonify({'error': 'User not found'}), 404
    try:
        schemes = json.loads(user.verified_schemes_data) if user.verified_schemes_data else []
...
def calculate_benefit_score(scheme):
    ...
    score = 0
    high_value_keywords = ['pension', 'scholarship', 'insurance', 'housing', 'subsidy', 'grant', 'financial aid', 'monthly', 'annual']
    medium_value_keywords = ['training', 'loan', 'credit', 'certificate', 'support', 'guidance']
    
    text = (scheme.name + " " + (scheme.benefits or "") + " " + (scheme.description or "")).lower()
    ...
    if scheme.category:
        ...
        if 'pension' in cat or 'financial' in cat: score += 15
        if 'education' in cat: score += 10
...
def lifecycle_forecast():
    ...
    if not user or user.age is None:
        return jsonify({'error': 'Please complete your profile first (Age is required)', 'incomplete_profile': True}), 200
    ...
    class LifecycleUser:
        ...
        def __init__(self, real_user, age_override=None):
            ...
            self._age = age_override if age_override is not None else (real_user.age or 0)
    ...
    try:
        ...
        ranked_current, _ = orch.evaluate_all(current_user, all_schemes, use_cache=False)
        current_eligible_ids = {
            int(r.scheme_id) for r in ranked_current if r.confidence >= DASHBOARD_THRESHOLD
        }
        ...
        for years_ahead in [1, 2, 3, 5]:
            future_user = LifecycleUser(user, age_override=(user.age or 0) + years_ahead)
            ranked_future, _ = orch.evaluate_all(future_user, all_schemes, use_cache=False)
            future_ids = {
                int(r.scheme_id) for r in ranked_future if r.confidence >= DASHBOARD_THRESHOLD
            }
            ...
            forecast_results['milestones'].append({
                ...
                'topScheme': newly_eligible_schemes[0]['name'] if newly_eligible_schemes else None,
                'years': years_ahead
            })
        ...
        import traceback
        logger.error(traceback.format_exc())
        return jsonify({'error': 'Could not generate forecast', 'milestones': [], 'now': {'count': 0, 'score': 0}}), 200
...
def audit_init():
    ...
    from sqlalchemy import or_
    schemes = Scheme.query.filter(or_(
        Scheme.name.ilike(f"%{query}%"),
        Scheme.category.ilike(f"%{query}%")
    )).limit(5).all()

    if not schemes:
        return jsonify({'error': 'No matching scheme found. Please try a different name.'}), 404
    ...
    try:
        ...
        if start != -1 and end != -1:
            ...
            try:
                ...
                q_text = re.sub(r'^```(?:json)?|```$', '', raw_text, flags=re.MULTILINE).strip()
        ...
        if "429" in err_msg or "quota" in err_msg.lower():
             return jsonify({'error': 'AI rate limit reached. Please wait 10 seconds and try again.'}), 429
...
EDU_LEVEL_RANK = {
    ...
    "primary": 1, "class 5": 1,
    "upper primary": 2, "class 8": 2, "middle school": 2,
    "secondary": 3, "class 10": 3, "sslc": 3, "matric": 3, "10th": 3,
    "higher secondary": 4, "class 12": 4, "hsc": 4, "12th": 4, "intermediate": 4, "puc": 4,
    "diploma": 5, "itc": 5, "iti": 5, "vocational": 5,
    "graduate": 6, "graduation": 6, "ug": 6, "bachelor": 6, "degree": 6,
    "post-graduate": 7, "postgraduate": 7, "pg": 7, "master": 7, "msc": 7, "mba": 7,
    "phd": 8, "doctorate": 8, "m.phil": 8,
}
...
DOC_TEXT_KEYWORDS = {
    ...
    "income certificate": "income_certificate", "income cert": "income_certificate",
    "caste certificate": "caste_certificate", "community certificate": "caste_certificate",
    "disability certificate": "disability_certificate", "pwd certificate": "disability_certificate",
    "ration card": "ration_card", "bpl card": "ration_card",
    "bank passbook": "bank_passbook", "bank account": "bank_passbook",
    "domicile certificate": "domicile_certificate", "residence certificate": "domicile_certificate",
    "marks card": "marks_card", "marksheet": "marks_card",
    "degree certificate": "degree_certificate",
    "bonafide": "bonafide_certificate",
    "land records": "land_records", "patta": "land_records",
    "sports certificate": "sports_certificate",
    "ncc certificate": "ncc_certificate", "ncc card": "ncc_certificate",
    "nss certificate": "nss_certificate",
    "arts certificate": "arts_certificate", "cultural certificate": "arts_certificate",
    "merit certificate": "merit_certificate",
    "farmer id": "farmer_id", "kisan credit": "farmer_id",
    "birth certificate": "birth_certificate",
    "marriage certificate": "marriage_certificate",
    "widow certificate": "widow_certificate",
}
...
def reject_all_pending_schemes():
    ...
    db.session.commit()
    return jsonify({'message': f'Successfully rejected all {rejected_count} schemes'}), 200
...
def submit_feedback():
    ...
    existing = ApplicationFeedback.query.filter_by(
        user_id=user_id, scheme_id=scheme_id).first()
    ...
    if existing:
        ...
        record = ApplicationFeedback(
            ...
            follow_up_due        = (datetime.utcnow() + timedelta(days=30)
                                    if did_apply == 'Yes' else None),
        )
...
def submit_outcome(feedback_id):
    ...
    record = ApplicationFeedback.query.filter_by(
        id=feedback_id, user_id=user_id).first()
    if not record:
        return jsonify({'error': 'Feedback record not found'}), 404

    data = request.get_json() or {}
    record.outcome              = data.get('outcome')        # Approved|Rejected|Pending|Withdrew
    record.rejection_reason_code = data.get('rejectionReasonCode')  # income|caste|age|docs|other
...
def get_my_feedback():
    ...
    if not user_id:
        return jsonify({'error': 'Not logged in'}), 401
    records = ApplicationFeedback.query.filter_by(user_id=user_id)\
        .order_by(ApplicationFeedback.created_at.desc()).all()
...
def _maybe_update_scheme_confidence(scheme_id: int):
    ...
    reason_counts = Counter(r.rejection_reason_code for r in outcomes if r.rejection_reason_code)
...
def get_accuracy_analytics():
    ...
    for cat, counts in sorted(cat_stats.items()):
        ...
        acc = (counts['approved'] / total_outcomes * 100) if total_outcomes >= 3 else None
    ...
    scheme_rejection_counts = Counter(
        (f.scheme_id, f.scheme_name) for f in rejected)
    top_false_positives = [
    ...
    scheme_approval_counts = Counter(
        (f.scheme_id, f.scheme_name) for f in approved)
    top_accurate = [
    ...
    avg_approved_score = round(sum(approved_scores)  / len(approved_scores),  1) if approved_scores  else None
    avg_rejected_score = round(sum(rejected_scores)  / len(rejected_scores),  1) if rejected_scores  else None
    ...
    return jsonify({
        'summary': {
            ...
            'approvalRatePct':    round(approval_rate * 100, 1) if approval_rate is not None else None,
...
class ApplicationFeedback(db.Model):
    ...
    rejection_reason_code = db.Column(db.String(50)) # income | caste | age | docs | other
    ...
    def to_dict(self):
        return {
            'id': self.id, 'schemeId': self.scheme_id, 'schemeName': self.scheme_name,
            'category': self.category, 'didApply': self.did_apply,
            'reasonNotApplied': self.reason_not_applied, 'outcome': self.outcome,
            ...
            'followUpDue': self.follow_up_due.isoformat() if self.follow_up_due else None,
        }
    ...
    with app.app_context():
        ...
        try:
            ...
            if 'application_feedback' in insp.get_table_names():
                fb_cols = [col['name'] for col in insp.get_columns('application_feedback')]
                fb_new_cols = {
