...
class NewEligibilityOrchestrator:
    ...
    def evaluate_single_scheme(self, scheme, user, use_cache=True, user_disqualifications=None):
        """
        Evaluate one scheme for one user.
        Exclusively uses the multi-pass engine for 100% stability.
        user_disqualifications: Optional set of scheme_ids that are already disqualified.
        """
        ...
        # ANSWER-AWARE DISQUALIFICATION CHECK (Optimized)
        if user_disqualifications is not None:
            if scheme.id in user_disqualifications:
                return EligibilityOutput(
                    result=INELIGIBLE, confidence=1.0,
                    blocking_reason="Disqualified based on your previous answers.",
    ...
    def evaluate_all(self, user, schemes, use_cache=True, question_cap=None):
        """
        Evaluate all schemes for a user.
        Returns (ranked_list, questions_list).
        question_cap=None means NO CAP - all questions returned.
        """
        if not hasattr(user, 'get_profile_dict'):
            profile = {}
        else:
        ...
        # PRE-FETCH DISQUALIFICATIONS
        user_disqualifications = set()
        try:
            from app import SchemeClarification
            disq = SchemeClarification.query.filter_by(user_id=user.id, ai_verdict='INELIGIBLE').all()
            user_disqualifications = {d.scheme_id for d in disq}
        except:
            pass
        ...
        # BATCH SIZE for progress logging if needed
        for scheme in candidates:
            eo = self.evaluate_single_scheme(scheme, user, use_cache, user_disqualifications=user_disqualifications)
...
def build_engine_response(orch, user, all_schemes):
    ...
    """
    ranked, questions = orch.evaluate_all(user, all_schemes, use_cache=True, question_cap=None)
    ...
    return {
        ...
        "meta": {
            ...
            "ineligible":      sum(1 for r in ranked if r.result == INELIGIBLE),
        }
    }
