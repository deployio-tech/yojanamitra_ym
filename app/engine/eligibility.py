...
def _margin_bonus(conditions, user_profile) -> float:
    ...
    for cond in conditions:
        ...
        try:
            ...
            logging.warning(f"[ELIGIBILITY] Invalid numeric value for '{cond.field}': '{pv}' - treating as missing")
            continue
...
class EligibilityEngine:
    ...
    def evaluate(self, scheme, user_profile: dict,
        ...
        for field_name, field_conds in by_field.items():
            ...
            if field_name in NUMERIC_FIELDS:
                ...
                try:
                    ...
                    pv = None  # treat as UNKNOWN, not crash

                if gte_vals or lte_vals:
                    if pv is not None and max_lower <= pv <= min_upper:
        ...
        # REFINED: Any failure (hard or soft) on an answerable field means NOT ELIGIBLE.
        user_hard_fail = False
        user_any_fail = False
        user_hard_missing = (len(unmapped) > 0)
        ...
        for r in all_results:
            ...
            if status in ["fail", "false"]:
                user_any_fail = True
                if getattr(r, "condition_type", "soft") == "hard":
                    user_hard_fail = True
            elif status in ["missing", "unknown", "none", ""]:
                # Only hard missing counts for forcing POSSIBLE status
                if getattr(r, "condition_type", "soft") == "hard":
                    user_hard_missing = True
        
        if user_hard_fail:
            # RULE 1: Any HARD USER FAIL → INELIGIBLE
            result = INELIGIBLE
        elif user_any_fail:
            # RULE 2: Any SOFT FAIL → INELIGIBLE or POSSIBLE based on confidence
            # For now, let's treat any failure as INELIGIBLE to be safe, 
            # unless it's very low confidence.
            result = INELIGIBLE
        elif user_hard_missing:
            # RULE 3: No Fail, but hard info is missing → POSSIBLE
            result = POSSIBLE
        else:
            # RULE 4: No Fail, No Missing Hard Info → FULLY ELIGIBLE
            result = ELIGIBLE
