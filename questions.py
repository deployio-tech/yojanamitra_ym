...
class QuestionEngine:
    ...
    def _is_answered_in_profile(self, field_name: str, user_profile: dict) -> bool:
        """
        Returns True if the field (or any of its canonical aliases) has a value in the profile.
        Checks: direct field name, canonical form, and all form_to_canonical aliases.
        """
        # 1. Direct match
        if user_profile.get(field_name) is not None:
            return True
        # 2. Via normalizer canonical
        canonical = self._normalizer.form_to_canonical.get(field_name, field_name)
        if user_profile.get(canonical) is not None:
            return True
        # 3. All form aliases that map to the same canonical
        for form, canon in self._normalizer.form_to_canonical.items():
            if canon == canonical and user_profile.get(form) is not None:
                return True
        # 4. Check CANONICAL_GROUPS from eligibility module
        from app.engine.eligibility import CANONICAL_GROUPS, FIELD_TO_CANONICAL
        group_key = FIELD_TO_CANONICAL.get(field_name, field_name)
        group_fields = next(
            (fields for gk, fields in CANONICAL_GROUPS.items() if gk == group_key),
            []
        )
        for gf in group_fields:
            if user_profile.get(gf) is not None:
                return True
        return False
    ...
    def select_questions(
        ...
        for scheme, elig_out in possible_schemes:
            for field in elig_out.missing_fields:
                canonical = self._normalizer.form_to_canonical.get(field, field)
                all_missing.append({
                    "field": field,
                    "canonical": canonical,
                    "scheme": scheme,
        ...
        filtered = [
            ...
            if (
                not self._is_answered_in_profile(item["field"], user_profile)
                and item["canonical"] not in session_answered
                and is_user_answerable(item["field"])
            )
        ]
        ...
        for item in filtered:
            if item["canonical"] not in seen_canonicals:
                seen_canonicals.add(item["canonical"])
                deduped.append(item)

        grouped = {}
        for item in deduped:
            concept = get_concept_for_field(item["canonical"])
            if not concept:
                concept = item["canonical"]
            if concept == "other":
                concept = item["canonical"]
            concept = concept.strip().lower()
            
            if concept not in grouped:
                grouped[concept] = {"fields": set(), "schemes": set()}
            grouped[concept]["fields"].add(item["field"])
            grouped[concept]["schemes"].add(item["scheme"].id)

        def infer_field_type(fields):
            for f in fields:
                if "income" in f or "amount" in f or "age" in f:
                    return "number"
                if f.startswith("is_") or f.startswith("has_") or f.startswith("does_"):
                    return "boolean"
            return "single_choice"

        questions = []
        for concept, data in grouped.items():
            rep_field = sorted(data["fields"])[0]
            field_type = infer_field_type(data["fields"])
            text = QUESTION_TEMPLATES.get(concept) or f"Please confirm: {concept.replace('_', ' ')}"
            
            from app.engine.normalize import get_concept_options
            concept_opts = get_concept_options(concept) or []

            questions.append(Question(
                question_id=f"q_{concept}",
                field=rep_field,
                concept=concept,
                text=text,
        ...
        def compute_impact_score(question, possible_schemes):
            hard = 0
            soft = 0
            q_field = get_canonical_field(question.field)
            for scheme, elig_out in possible_schemes:
                missing_set = {get_canonical_field(f) for f in elig_out.missing_fields}
                if q_field not in missing_set:
                    continue
                for cond in scheme.conditions:
                    if get_canonical_field(cond.field) == q_field:
                        if getattr(cond, "condition_type", "hard") == "hard":
                            hard += 1
                        else:
                            soft += 1
            return hard * 3 + soft
        
        questions.sort(key=lambda q: compute_impact_score(q, possible_schemes), reverse=True)
        MAX_QUESTIONS = 40
        return questions[:MAX_QUESTIONS] if self.max_q is None else questions[:self.max_q]
